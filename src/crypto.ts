// AES-256-GCM file encryption helpers using the browser Web Crypto API.
//
...
/** Encrypt `bytes` with `password`. Returns ciphertext and metadata needed for decryption. */
export async function encryptFile(
  ...
  const { key, raw } = await deriveKey(password, salt);
...
/**
 * Verify a candidate password against the stored passwordHash without decrypting.
...
export async function verifyPassword(
  ...
  return (await hashRaw(raw)) === encMeta.passwordHash;
}

/**
 * Decrypt ciphertext using the given password and stored encryption metadata.
 * Throws if the password is wrong or the data is tampered (AES-GCM authentication).
 */
